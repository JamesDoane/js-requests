...
const makeQuery = (event) => {
    //YOUR CODE HERE
    let str = event.target.textContent
    axios.get(`${baseURL}/query-test/?foods=${str}`)
        .then((res)=>{
            console.log(res.data)
        })
}
...
/*
    ...
    The console logged you sent no query

    2: Send more than 1 query on the URL -- what happened? 
    the console logged "you sent more than one query"
*/
    ...
    axios.post(`${baseURL}/food`,body)
        .then((res)=>{
            console.log(res.data)
                ...
                let p = document.createElement('p')
                p.textContent = res.data[res.data.length-1]
                document.querySelector('body').appendChild(p)
              
            
        })
}
document.getElementById('submitFood').addEventListener('click',createFood)
// CODE HERE 
